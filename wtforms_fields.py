...
class RegistrationForm(FlaskForm):
    ...
    confirm_pswd = PasswordField('confirm_pswd_label',  validators=[InputRequired(message='Confirm password'), EqualTo('password', message="Passwords must match")])
    submit_button = SubmitField('Create')

    def validate_username(self, username):
        user_object = User.query.filter_by(username=username.data).first()
        if user_object:
            raise ValidationError("User already exists. Select a different username.")
